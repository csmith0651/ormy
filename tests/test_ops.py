...
class TestOpNode:
    def test_find_in_parents(self, tmpdir):
        ...
        q = db.query(ModelForTest).field('intcol').eq(50)
...
class TestListResultOpNode:
    ...
    def test_lt(self, tmpdir):
        db = CsvDatabase(str(tmpdir))
        lt = db.query(ModelForTest).field('intcol').lt(50)
    ...
    def test_lte(self, tmpdir):
        db = CsvDatabase(str(tmpdir))
        lte = db.query(ModelForTest).field('intcol').lte(50)
    ...
    def test_gt(self, tmpdir):
        db = CsvDatabase(str(tmpdir))
        gt = db.query(ModelForTest).field('intcol').gt(50)
    ...
    def test_gte(self, tmpdir):
        db = CsvDatabase(str(tmpdir))
        gte = db.query(ModelForTest).field('intcol').gte(50)
...
""")
        db = CsvDatabase(str(tmpdir))
        results = db.query(ModelForTest).field('intcol').eq(50).eval()
